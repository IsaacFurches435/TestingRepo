...
public class JoystickAxis {
    ...
    public double get() {
        ...
        retriveDeadband(command, m_deadband);
    ...
    public static double retriveDeadband(double command, double m_deadband) {
    ...
    public double retriveDeadband(double m_deadband) {
        command = m_controller.getRawAxis(m_axis);
        if(command > m_deadband) {
            command = (1 / (1 - m_deadband)) * command - (m_deadband / (1 - m_deadband));
        } else if (command < -m_deadband) {
            command = (1 / (1 - m_deadband)) * command + (m_deadband / (1 - m_deadband));
